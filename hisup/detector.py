...
class BuildingDetector(nn.Module):
    def __init__(self, cfg, test=False):
        ...
        #height head
        self.height_head = self._make_conv(dim_in, dim_in, dim_in)
        ...
        self.height_predictor = self._make_predictor(dim_in, 1)
    ...
    def forward_test(self, images, annotations = None):
        ...
        mask_pred_ = self.mask_predictor(mask_feature + mask_att_feature)
        jloc_pred_ = self.jloc_predictor(jloc_feature + jloc_att_feature)
        afm_pred_ = self.afm_predictor(afm_feature)

        afm_conv = self.refuse_conv(afm_pred_)
        remask_pred_ = self.final_conv(torch.cat((features, afm_conv), dim=1))

        joff_pred = outputs[:, :].sigmoid() - 0.5
        mask_pred = mask_pred_.softmax(1)[:,1:]
        jloc_convex_pred = jloc_pred_.softmax(1)[:, 2:3]
        jloc_concave_pred = jloc_pred_.softmax(1)[:, 1:2]
        remask_pred = remask_pred_.softmax(1)[:, 1:]
        ...
        extra_info = {"np_remask_pred":remask_pred[0,0,:,:].cpu().numpy().round(),"np_remask_feature":remask_pred_[0,1,:,:].cpu().numpy()}
        output = {
    ...
    def forward_train(self, images, annotations = None):
        ...
        extra_info = {"jloc_pred":jloc_pred,"outputs":outputs[:, :],"mask_pred":mask_pred,"afm_pred":afm_pred,"remask_pred":remask_pred}
...
def get_pretrained_model(cfg, dataset, device, pretrained=True):
    PRETRAINED = {
        'crowdai': r'D:\PROJELER\hisup\hisup\models\crowdai_hrnet48_e100.pth',
